...
func TestStatefulConnReadFromHasBufferAvailable(t *testing.T) {
	preData := []byte("head data")
	rawData := strings.Repeat("helloworld", 1)
	tailData := []byte("tail data")
	data := strings.NewReader(rawData)
	...
	reader := conn.(*statefulConn).Conn.(*Conn)
...
func TestStatefulConnReadFromNoBufferAvailable(t *testing.T) {
	rawData := strings.Repeat("helloworld", 1)
	tailData := []byte("tail data")
	data := strings.NewReader(rawData)
	c := &mockConn{}
...
func TestStatefulConnPeekRelease(t *testing.T) {
	c := &mockConn{}
	conn := NewStatefulConn(context.Background(), newConn(c, 4096))
	b, _ := conn.Peek(1)
	if len(b) != 1 {
	...
	// test maxSize
	if conn.(*statefulConn).Conn.(*Conn).maxSize != 524288 {
		t.Errorf("unexpected maxSize: %v, expected 524288", conn.(*statefulConn).Conn.(*Conn).maxSize)
	}
}

func TestStatefulConnReadBytes(t *testing.T) {
	c := &mockConn{}
	conn := newConn(c, 4096)
	b, _ := conn.Peek(1)
	if len(b) != 1 {
