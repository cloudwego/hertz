...
func (d *sliceDecoder) Decode(req *protocol.Request, params param.Params, rv reflect.Value) (bool, error) {
	...
	if len(ss) == 0 {
		if !jsonExist && len(d.defaultv) > 0 {
			...
			f := d.FieldSetter(rv)
			err = json.Unmarshal(bytesconv.S2b(d.defaultv), f.Value().Addr().Interface())
			if err != nil {
				f.Reset()
	...
	if err != nil {
		...
		// Fallback: if individual element decoding fails, try to decode the first string
		// as a complete JSON array. This handles cases where the input is a JSON-encoded
		// array string rather than individual values.
		// This behavior preserves backward compatibility with the previous implementation.
		err1 := json.Unmarshal(bytesconv.S2b(ss[0]), v.Addr().Interface())
		if err1 != nil {
			f.Reset()
			return false, fmt.Errorf("element decode failed: %w, JSON fallback also failed: %v", err, err1)
		}
	}
	return true, nil
}
...
func (d *sliceDecoder) decodeElem(rv reflect.Value, s string, req *protocol.Request, params param.Params) error {
	...
	if s == "" && d.c.LooseZeroMode {
		rv.Set(reflect.Zero(rt)) // use SetZero() >= go1.20
		return nil
	}

	if d.elemHasUnmarshalText && tryTextUnmarshaler(rv.Addr(), s) {
		// if TextUnmarshaler not available
		// it will fallback to default decoder for backward compatibility
		return nil
	}
