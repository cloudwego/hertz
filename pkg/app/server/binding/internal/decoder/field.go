...
type fieldInfo struct {
	...
	elemHasUnmarshalText bool // for slice
...
func (f *fieldInfo) init() {
	...
	f.hasUnmarshalText = reflect.PointerTo(rt).Implements(textUnmarshalerType)
	if f.IsRepeated() {
		f.elemDecodeFunc = c.TypeUnmarshalFuncs[dereferenceType(rt.Elem())]
		f.elemHasUnmarshalText = reflect.PointerTo(rt.Elem()).Implements(textUnmarshalerType)
	}

	// Enable unsafe string conversion for basic types (int, float, etc.) that are
	// immediately parsed and don't retain references to the underlying byte slice.
	// This optimization avoids unnecessary memory allocations.
	f.unsafestr = getBaseDecodeByKind(rt.Kind()) != nil
	if rt.Kind() == reflect.String {
		f.unsafestr = false // strings retain references to the value, must copy to prevent data races
	}
	if f.decodeFunc != nil || f.elemDecodeFunc != nil {
		f.unsafestr = false // custom decode functions may retain references, use safe copying
	}
}
