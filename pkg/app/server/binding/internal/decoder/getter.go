...
// unsafeStr converts a byte slice to string, optionally without copying.
// When fi.unsafestr is true, it performs a zero-copy conversion using bytesconv.B2s.
// This is safe for basic types (int, float, etc.) where the string is immediately
// parsed and not retained. For strings and custom types, copying is always used
// to prevent data races if the underlying byte slice is modified.
