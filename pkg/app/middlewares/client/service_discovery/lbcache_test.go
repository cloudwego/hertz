...
var defaultOptions = loadbalance.LoadBalanceOptions{
	RefreshInterval: loadbalance.DefaultRefreshInterval,
	ExpireInterval:  loadbalance.DefaultExpireInterval,
}
...
func TestBuilder(t *testing.T) {
	...
	NewBalancerFactory(r, WithLoadBalanceOptions(lb, loadbalance.LoadBalanceOptions{}))
	...
	ins, err := b.(*BalancerFactory).GetInstance(context.TODO(), req)
	assert.Assert(t, err == nil)
	assert.Assert(t, ins.Address().String() == "127.0.0.1:8888")
	assert.Assert(t, ins.Weight() == 10)
	value, exists := ins.Tag("a")
