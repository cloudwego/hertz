...
func TestHostClient(t *testing.T) {
	...
	tr := &HostClient{ClientOption: &ClientOption{TLSConfig: tlsConfigInsecure, Addr: u.Host, KeepAlive: true, Dialer: standard.NewDialer(), IsTLS: true}}
...
func testHostClientReusesConns(t *testing.T, wantSame bool, modReq func(*protocol.Request)) {
	...
	tr := &HostClient{
		ClientOption: &ClientOption{
			TLSConfig: tlsConfigInsecure, Addr: u.Host, KeepAlive: true,
...
func TestHostClientAbortClosesPipes(t *testing.T) {
	...
	go func() {
		...
		tr := &HostClient{ClientOption: &ClientOption{
			TLSConfig: tlsConfigInsecure, Addr: u.Host, KeepAlive: true,
			Dialer: standard.NewDialer(),
			IsTLS:  true,
		}}
...
func TestHostClientChecksRequestHeaderListSize(t *testing.T) {
	...
	tr := &HostClient{ClientOption: &ClientOption{
		TLSConfig: tlsConfigInsecure, MaxHeaderListSize: 10 << 20, KeepAlive: true,
		Dialer: standard.NewDialer(),
		IsTLS:  true,
	}}
	defer tr.CloseIdleConnections()
...
func TestClientConnPing(t *testing.T) {
	...
	tr := &HostClient{ClientOption: &ClientOption{TLSConfig: tlsConfigInsecure, Addr: u.Host, IsTLS: true, KeepAlive: true, Dialer: standard.NewDialer()}}
	defer tr.CloseIdleConnections()
...
func TestHostClientNewClientConnCloseOnWriteError(t *testing.T) {
	tr := &HostClient{ClientOption: &ClientOption{KeepAlive: true}}
...
func TestClientConnReservations(t *testing.T) {
	cc := &clientConn{
		...
		nextStreamID:         1,
		hc:                   &HostClient{ClientOption: &ClientOption{KeepAlive: true}},
	}
